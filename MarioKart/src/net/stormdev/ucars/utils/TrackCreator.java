...
public class TrackCreator {
	...
	public void setExit(){
		...
		player.sendMessage(main.colors.getInfo() + main.msgs.get("setup.create.grid"));
		stage = 2;
		return;
	}
	public void addGrid(){
		track.addToStartGrid(player.getLocation());
		player.sendMessage(main.colors.getInfo()+"["+track.getStartGrid().size()+"]");
		return;
	}
	public void finishGrid(){
		player.sendMessage(main.colors.getInfo() + main.msgs.get("setup.create.checkpoints"));
		stage = 3;
		return;
	}
	...
	public void finishCheckpoints(){
		...
		player.sendMessage(main.colors.getInfo()+main.msgs.get("setup.create.line1"));
		stage = 4;
		return;
	}
	public void setLine1(){
		track.setLine1(player.getLocation());
		player.sendMessage(main.colors.getInfo()+main.msgs.get("setup.create.line2"));
		stage = 5;
		return;
	}
