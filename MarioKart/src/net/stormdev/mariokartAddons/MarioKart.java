			...
			if(ItemStackFromId.equals(main.config.getString("mariokart.random"), inHand.getTypeId(), inHand.getDurability())){
					...
					public void run() {
						...
						while(amount > 0){
							if(ucars.listener.inACar(player)){
						        player.getLocation().getWorld().playSound(player.getLocation(), Sound.BURP, 3, 1);
							}
						try {
							Thread.sleep(3000);
				...
				for(int i=0;i<pls.length;i++){
					...
					if(pl.getVehicle() instanceof Minecart){
						...
						ucars.listener.carBoost(pl.getName(), power, 8000, ucars.config.getDouble("general.cars.defSpeed"));
					}
					}
					}
				}
					...
					public void run() {
						...
						while (count > 0){
							for(int i=0;i<pls.length && i<=ppos;i++){
								Player pl = plugin.getServer().getPlayer((String) pls[i]);
								pl.sendMessage(main.colors.getTitle()+"[MarioKart:] "+main.colors.getInfo()+count);
							}
							try {
								Thread.sleep(1000);
							} catch (InterruptedException e) {
						...
						plugin.getServer().getScheduler().runTask(plugin, new Runnable(){
							public void run() {
								for(int i=0;i<pls.length && i<ppos;i++){
									...
									if(pl.getVehicle() != null){
									if(pl.getVehicle() instanceof Minecart){
										if(!pl.getVehicle().hasMetadata("car.braking") && !pl.getVehicle().hasMetadata("kart.immune")){
