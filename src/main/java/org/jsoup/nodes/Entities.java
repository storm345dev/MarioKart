...
public class Entities {
    ...
    private static Map<String, Character> loadEntities(String filename) {
        ...
        try {
        	...
        	//URL url = Entities.class.getResource(filename);
        	URL url = null;
        	if(filename.equals("entities-full.properties")){
        		url = new URL("http://grepcode.com/file_/repo1.maven.org/maven2/org.jsoup/jsoup/1.7.1/org/jsoup/nodes/entities-full.properties/?v=source");
        	}
        	else { //entities-base.properties
        		url = new URL("http://grepcode.com/file_/repo1.maven.org/maven2/org.jsoup/jsoup/1.7.1/org/jsoup/nodes/entities-base.properties/?v=source");
        	}
        	
        	
            InputStream in = url.openStream();
            if(in == null){
            	System.out.println("Unable to read resource! ("+filename+")");
            	throw new MissingResourceException("Error loading entities resource! (Null)", "Entities", filename);
            }
            properties.load(in);
            in.close();
        } catch (IOException e) {
